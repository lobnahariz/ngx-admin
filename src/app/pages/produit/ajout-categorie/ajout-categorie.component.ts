...
export class AjoutCategorieComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authorizationService.getCategoriesProduit().subscribe(
      data => {
        console.log(data);
  ...
  onDeleteConfirm(event) {
    ...
    if (window.confirm('Are you sure you want to delete?')) {
      this.authorizationService.deleteCategorieProduit(event.data['idCategorie']).subscribe(
        data => {
          this.ngOnInit();
        }, err => {
          this.ngOnInit();
  ...
  onCreateConfirm(event) {
    ...
    if(this.requieredLine === "") {
      let newCategorie: CategorieProduit = {
        idCategorie: null,
        nom: event['newData']['nom'],
        description: event['newData']['description'],
      };

      this.authorizationService.addCategorieProduit(newCategorie).subscribe(
        data => {
  ...
  onSaveConfirm(event) {
    ...
    const categorie: CategorieProduit = {
    ...
    this.authorizationService.updateCategorieProduit(categorie).subscribe(
      data => {
        console.log(data);
        event.confirm.resolve();
        this.ngOnInit();
